...
uint8_t bcm2835_i2cbb_read(struct bcm2835_i2cbb *bb, uint8_t ack) {
	...
	for (j = 0; j < 8; j++) {
		data <<= 1;      // shift in
		bcm2835_i2cbb_bitdelay(bb->clock_delay);
		bcm2835_i2cbb_sclH(bb);
		;      // set clock high to get data
		bcm2835_i2cbb_bitdelay(bb->clock_delay); // delay for slave
		if (bcm2835_gpio_lev(bb->sda))
			data++;   // get data
...
// *****************************************************************************
void bcm8235_i2cbb_discover(struct bcm2835_i2cbb *bb, uint8_t first,
		uint8_t last) {
	int i, j;
	int res;
